...
maximizes the True Skill Statistic (TSS), as suggested by @Carlson2020EmbSpe. 
...
measured species richness per site as the sum of each row, i.e. the number of
species present. We removed the sites without any species from the predicted
community matrix (this was not necessary for the observed community matrix, as
it was, by design, only composed of sites with at least one species present)
and applied the Hellinger transformation to our $Y$ matrices, as recommended by
...
while low values indicate a more common species set. We note that our LCBD
...
relative to the maximum value from each matrix Y, meaning that the new
maximum value was 1, and all other values represented fractions.
